...
unsigned int  VEDI =  0x0A7F ;   //电流放电保护阈值
unsigned int  VPTH =  0x03;       //ADC采集次数
unsigned int  Delay_count = 0x0A;//过流之后，恢复采集，延时时间
...
unsigned int EEPROM_Read_value_Delay_Count = 0;
...
void Overcurrent_Control()
{
  ...
  Voltage0 = Voltage0/VPTH;
  //printf("---- Voltage0 = :0x%04X\r\n",Voltage0);
  if(Voltage0 > VEDI)       //过流  1.64V
...
void Overvoltage_Control() 
{
    ...
    Voltage1 = Voltage1/VPTH;           
   //printf("---- Voltage1 = :0x%04X\r\n",Voltage1);
   if(Voltage1 > 0x0E14)      //过压 2.2V
...
//    IAP_Erase(0x0600);
    ...
    EEPROM_Read_value_Delay_Count   = IAP_Read(0x0600);

   if((EEPROM_Read_value_VEDI_H == 0xFFFF) || (EEPROM_Read_value_VPTH == 0xFFFF)|| (EEPROM_Read_value_Delay_Count == 0xFFFF))
    {   
       IAP_Erase(0x0000);
       ...
       IAP_Erase(0x0600);        
       IAP_Program(0x0600,10);  
       EEPROM_Read_value_Delay_Count = IAP_Read(0x0600);
      
       printf("Wirted VEDI = %d     %d     %d     %d\r\n",EEPROM_Read_value_VEDI_H,EEPROM_Read_value_VEDI_L,EEPROM_Read_value_VPTH,EEPROM_Read_value_Delay_Count);     
       printf("first write here\r\n");
       EEPROM_Read_value_VEDI_H=0;
       EEPROM_Read_value_VEDI_L=0;
       EEPROM_Read_value_VPTH = 0;
       EEPROM_Read_value_Delay_Count = 0;
    }
    else
    {
      ...
      Delay_count = EEPROM_Read_value_Delay_Count;  
      printf("courrt VEDI = 0x%04X\r\n",VEDI); 
      printf("courrt VPTH = 0x%04X\r\n",VPTH);      
      printf("courrt Delay_count = 0x%04X\r\n",Delay_count);
      EEPROM_Read_value_VEDI_H=0;
      EEPROM_Read_value_VEDI_L=0;
      EEPROM_Read_value_VPTH = 0;
      EEPROM_Read_value_Delay_Count = 0;
...
void EEPROM_Program(void)
{
   if(KEY0_P32_Down || KEY1_P33_Down || KEY2_P34_Down || KEY3_P35_Down|| KEY4_P36_Down || KEY5_P37_Down)
   {
     ...
     else if(KEY2_P34_Down)
     {
        ...
        EEPROM_Read_value_VPTH = IAP_Read(0x0400);
        printf("courrt VPTH = 0x%04X\r\n",VPTH);
        printf("courrt EEPROM_Read_value_VPTH = 0x%04X\r\n",EEPROM_Read_value_VPTH);
        EEPROM_Read_value_VPTH =0;
     }
     else if(KEY3_P35_Down)
     {
        ...
        EEPROM_Read_value_VPTH = IAP_Read(0x0400);
        printf("VPTH = 0x%04X\r\n",VPTH);
        printf("EEPROM_Read_value_VPTH = 0x%04X\r\n",EEPROM_Read_value_VPTH);
        EEPROM_Read_value_VPTH =0;
     }
     else if(KEY4_P36_Down)    
     {
        ...
        Delay_count++;
        IAP_Erase(0x0600); 
        IAP_Program(0x0600,Delay_count);  
        EEPROM_Read_value_Delay_Count = IAP_Read(0x0600);
        printf("Delay_count = 0x%04X\r\n",Delay_count);
        printf("EEPROM_Read_value_Delay_Count = 0x%04X\r\n",EEPROM_Read_value_Delay_Count);
        EEPROM_Read_value_Delay_Count =0;
      ...
      if(Rx_Cnt > 0)	//确认有数据
      {
          ...
          if((Rx_Cnt >= 6) && (RxBuf[1] == ' '))	//最短命令为6个字节
          {
              ...
              if(RxBuf[0] == 'R'&& RxBuf[1] == ' ')	//PC请求读取某一个阈值
              {
                ...
                else if(RxBuf[0] == 'V'&& RxBuf[1] == 'P'&& RxBuf[1] == 'T'&& RxBuf[1] == 'H')
                {
                  EEPROM_Read_value_VPTH = IAP_Read(0x0400);
                  printf("courrt EEPROM_Read_value_VPTH = 0x%04X\r\n",EEPROM_Read_value_VPTH);
                }
                else if(RxBuf[0] == 'D'&& RxBuf[1] == 'E'&& RxBuf[1] == 'L'&& RxBuf[1] == 'Y')
                {  
                  EEPROM_Read_value_Delay_Count = IAP_Read(0x0600);
                  printf("courrt EEPROM_Read_value_Delay_Count = 0x%04X\r\n",EEPROM_Read_value_Delay_Count);
                }
                else
                {
...
{ 
   ...
   if(RI)
   {
      ...
      RxBuf[Rx_Cnt] = SBUF;     //存储数据到缓冲区	save the data to RBUF
